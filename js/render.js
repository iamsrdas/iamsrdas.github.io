/* =========================================================================
   ...
   HOW CARDS WORK: every card (home page AND each list page) only shows an
   image, a date, and a title. Clicking a card opens a popup right there on
   the same page with the full text. See openModal()/closeModal() below.

   HOW THE HOME PAGE WORKS: it shows one stacked section per content type
   (Latest Projects, Latest Publications, Latest Blog Posts, Latest Awards,
   Latest Training, Latest from Gallery), each showing up to the 3 newest
   items — i.e. items [0], [1], [2] of that array in data.js.
   ========================================================================= */
...
/* ---- NAV LINKS shown in the header on every page ---- */
const NAV_LINKS = [
...
/* ---- header: brand + nav links (collapses to a toggle on mobile) ---- */
function renderHeader(active){
  ...
  el.innerHTML = `
    <div class="nav-inner">
      ...
      <nav class="nav-links" id="navLinks">${items}</nav>
    </div>`;
...
/* ---- footer: social icons only, plus copyright / institution row ---- */
  ...
  el.innerHTML = `
    <div class="container">
      ...
      <div class="foot-inner">
        <p>&copy; 2026 ${SITE.name} — built with <span style="color:var(--accent-2)">&hearts;</span> using Claude</p>
...
/* =========================================================================
   POPUP / MODAL SYSTEM — used by every card on every page (and the home
   page) to show the full story without leaving the page.
   ========================================================================= */
function renderModalShell(){
  if (document.getElementById('modalOverlay')) return; // already injected
...
function openModal({ eyebrow, title, meta, image, mediaHtml, body }){
  ...
  content.innerHTML = `
    ...
    ${mediaHtml ? mediaHtml : (image ? `<img src="${image}" alt="${title || ''}">` : '')}
...
/* ---- one shared card builder — every list (home + each page) uses this,
   so every card looks and sizes exactly the same. ---- */
function cardHTML({ label, image, date, title, onclick }){
  return `
    <div class="card" onclick="${onclick}" tabindex="0" role="button"
...
/* ---- opener functions: look up an item and show it in the popup ---- */
function openProjectModal(id){
  const p = projects.find(x => x.id === id);
  if (!p) return;
  openModal({
    eyebrow: `Project · ${p.date}`,
    title: p.title,
    meta: p.tags.map(t => `<span class="tag">${t}</span>`).join(''),
    image: p.cover,
    body: `
      ${p.media && p.media.length ? `<div class="detail-media">${p.media.map(m => mediaTag(m)).join('')}</div>` : ''}
      ${p.description}
      ${p.links && (p.links.repo || p.links.demo) ? `<p style="margin-top:16px">
...
function openBlogModal(id){
  const b = blogPosts.find(x => x.id === id);
  if (!b) return;
  openModal({
    eyebrow: `Blog · ${b.date}`,
    title: b.title,
...
function openTrainingModal(index){
  const t = trainings[index];
  if (!t) return;
  openModal({
    eyebrow: `${t.org} · ${t.year}`,
    title: t.title,
    image: t.image,
    body: `<p>${t.description}</p>`
  });
}

function openGalleryModalItem(index){
  const g = gallery[index];
  if (!g) return;
  openModal({
    title: g.caption || '',
    mediaHtml: g.type === 'video'
      ? `<video src="${g.src}" controls autoplay style="width:100%;border-radius:10px;margin-bottom:16px"></video>`
      : `<img src="${g.src}" alt="${g.caption || ''}">`
  });
}

/* ---- shared builder for one "Latest ___" section on the home page ---- */
function latestBlock(label, viewAllHref, cardsHtmlArray){
  if (!cardsHtmlArray.length) return '';
  return `
    <div class="latest-section">
      <div class="section-head">
        <h2>${label}</h2>
        <a href="${viewAllHref}" class="mono">View all &rarr;</a>
      </div>
      <div class="grid cols-3">${cardsHtmlArray.join('')}</div>
    </div>`;
}

/* ---- HOME PAGE: hero + one stacked "Latest ___" section per content type,
   each showing up to the 3 newest items (array positions 0, 1, 2) ---- */
function renderHome(){
  const photo = document.getElementById('heroPhoto');
  if (photo) photo.src = SITE.photo;
  const nameEl = document.getElementById('heroName');
  if (nameEl) nameEl.textContent = `${SITE.name} - ${SITE.role}`;
  const roleEl = document.getElementById('heroRole');
  if (roleEl) roleEl.textContent = SITE.institution;
  ...
  if (!readout) return;
  readout.classList.remove('readout');
  readout.classList.add('latest-sections');

  const sections = [
    latestBlock('Latest Projects', 'projects.html',
      projects.slice(0, 3).map(p => cardHTML({
        label: p.tags && p.tags[0] ? p.tags[0] : 'Project',
        image: p.cover, date: p.date, title: p.title,
        onclick: `openProjectModal('${p.id}')`
      }))),
    latestBlock('Latest Publications', 'publications.html',
      publications.slice(0, 3).map((pub, i) => cardHTML({
        label: 'Publication', image: pub.image, date: pub.year, title: pub.title,
        onclick: `openPublicationModal(${i})`
      }))),
    latestBlock('Latest Blog Posts', 'blog.html',
      blogPosts.slice(0, 3).map(b => cardHTML({
        label: 'Blog', image: b.cover, date: b.date, title: b.title,
        onclick: `openBlogModal('${b.id}')`
      }))),
    latestBlock('Latest Awards', 'awards.html',
      awards.slice(0, 3).map((a, i) => cardHTML({
        label: 'Award', image: a.image, date: a.year, title: a.title,
        onclick: `openAwardModal(${i})`
      }))),
    latestBlock('Latest Training & Workshops', 'training.html',
      trainings.slice(0, 3).map((t, i) => cardHTML({
        label: 'Training', image: t.image, date: t.year, title: t.title,
        onclick: `openTrainingModal(${i})`
      }))),
    latestBlock('Latest from Gallery', 'gallery.html',
      gallery.slice(0, 3).map((g, i) => cardHTML({
        label: '', image: g.src, date: '', title: g.caption || 'Gallery',
        onclick: `openGalleryModalItem(${i})`
      })))
  ];

  readout.innerHTML = sections.join('') || emptyNote('updates');
}
...
function renderProjectsGrid(){
  ...
  el.innerHTML = projects.map(p => cardHTML({
    label: p.tags && p.tags[0] ? p.tags[0] : 'Project',
    image: p.cover, date: p.date, title: p.title,
    onclick: `openProjectModal('${p.id}')`
  })).join('');
...
function renderPublications(){
  ...
  el.innerHTML = publications.map((pub, i) => cardHTML({
    label: 'Publication', image: pub.image, date: pub.year, title: pub.title,
    onclick: `openPublicationModal(${i})`
  })).join('');
}
...
function renderBlogGrid(){
  ...
  el.innerHTML = blogPosts.map(b => cardHTML({
    label: 'Blog', image: b.cover, date: b.date, title: b.title,
    onclick: `openBlogModal('${b.id}')`
  })).join('');
}
...
function renderAwards(){
  ...
  el.innerHTML = awards.map((a, i) => cardHTML({
    label: 'Award', image: a.image, date: a.year, title: a.title,
    onclick: `openAwardModal(${i})`
  })).join('');
}
...
function renderTrainings(){
  ...
  el.innerHTML = trainings.map((t, i) => cardHTML({
    label: 'Training', image: t.image, date: t.year, title: t.title,
    onclick: `openTrainingModal(${i})`
  })).join('');
}

/* ---- GALLERY + lightbox (its own simple viewer, kept separate from the
   card popup system since gallery items are just photos/videos) ---- */

...
/* ---- NAV LINKS shown in the footer on every page ---- */
const NAV_LINKS = [
...
/* ---- header: just the brand/logo — page navigation now lives in the footer ---- */
...
/* ---- footer: every page link listed one by one, social icons, then copyright / institution row ---- */
function renderFooter(active){
  const el = document.getElementById('site-footer');
  if (!el) return;
  const navHtml = NAV_LINKS.map(l =>
    `<a href="${l.href}" class="footer-link ${l.key === active ? 'active' : ''}">${l.label}</a>`
  ).join('');
  el.innerHTML = `
    <div class="container">

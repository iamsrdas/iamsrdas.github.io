...
function renderHome(){
  ...
  const award = awards[0], training = trainings[0], pic = gallery[0];
  ...
  if (readout){
    readout.innerHTML = `
      ...
      ${pic ? `<a class="card-link" href="gallery.html"><div class="card">
        <img src="${pic.src}" alt="${pic.caption || ''}">
        <span class="label">Latest from Gallery</span><p>${pic.caption || ''}</p></div></a>` : ''}
    `;
  }
}
